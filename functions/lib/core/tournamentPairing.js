...
exports.buildEncounterCountsFromRounds = buildEncounterCountsFromRounds;
exports.generateTournamentPairings = generateTournamentPairings;
exports.generatePrecomputedTournamentSchedule = generatePrecomputedTournamentSchedule;
...
function addTableEncounters(encounterCounts, players) {
...
function addRoundEncounters(encounterCounts, tables) {
    for (const table of tables) {
        addTableEncounters(encounterCounts, table.playerIds);
    }
}
function scoreEncounterDistribution(encounterCounts) {
...
function isScoreBetter(candidate, baseline) {
    if (!baseline) {
        return true;
    }
...
function generateMinRepeatRoundPairings(playerIds, encounterCounts, rng) {
...
function buildEncounterCountsFromRounds(rounds) {
...
function generateTournamentPairings({ playerIds, standingsPoints, encounterCounts }) {
    ...
    const takeBestCandidate = (seedTable) => {
        let bestIndex = 0;
        let bestScore = Number.POSITIVE_INFINITY;
        for (let i = 0; i < available.length; i += 1) {
            const candidate = available[i];
...
function generatePrecomputedTournamentSchedule(input) {
