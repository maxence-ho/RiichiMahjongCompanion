...
export function GameCard({ id, participants, status, competitionIds }: GameCardProps) {
  return (
    <Link href={`/games/${id}`} className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Competition: {competitionIds.length ? competitionIds.join(', ') : 'Hors compétition'}
      </p>
    </Link>
  );
}
